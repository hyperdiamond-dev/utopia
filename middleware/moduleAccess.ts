...
export const moduleAccessMiddleware = createMiddleware(async (c, next) => {
  ...
  try {
    ...
    // Check module access
    const accessResult = await ModuleService.checkModuleAccess(
      userRecord.id,
      moduleName,
    );
  ...
  try {
    ...
    if (moduleData.isCompleted) {
      return c.json({
        ...
        message:
          "This module has already been completed and cannot be resubmitted",
      }, 400);
    }
      ...
      if (requestedModule && requestedModule !== currentModule.name) {
        ...
        if (!moduleData?.accessible) {
          return c.json({
            ...
            message:
              `Please complete "${currentModule.title}" before accessing other modules`,
