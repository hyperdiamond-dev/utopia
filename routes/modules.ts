...
modules.get('/current', authMiddleware, async (c) => {
  ...
  try {
    ...
    const moduleData = await ModuleService.getModuleForUser(userRecord.id, currentModule.name);
 ...
 */
modules.get('/:moduleName', authMiddleware, moduleAccessMiddleware, moduleReviewMiddleware, async (c) => {
 ...
 */
modules.post('/:moduleName/start', authMiddleware, moduleAccessMiddleware, async (c) => {
  const userRecord = c.get('userRecord');
  const moduleAccess = c.get('moduleAccess');
 ...
 */
modules.post('/:moduleName/save', authMiddleware, moduleAccessMiddleware, async (c) => {
  const userRecord = c.get('userRecord');
  const moduleAccess = c.get('moduleAccess');
 ...
 */
modules.post('/:moduleName/complete',
  authMiddleware,
  moduleAccessMiddleware,
  moduleCompletionMiddleware,
  async (c) => {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        return c.json({ error: 'Invalid submission data', details: error.errors }, 400);
      }
